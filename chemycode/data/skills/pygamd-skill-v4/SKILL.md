...
# 6. 智能可视化（可选，需要pip install ovito）
import subprocess
subprocess.run(["python", "smart_visualize.py", "traj.*.xml", "viz", "5"])
```
...
```python
import sys
sys.path.insert(0, r'<workspace_path>')  # 包含pygamd_gpu_init.py的目录路径
from pygamd_gpu_init import init_gpu
init_gpu()  # 返回True表示成功
```

`pygamd_gpu_init.py` 解决以下兼容性问题：
- numba不支持CUDA 13.2 → monkey-patch CTK_SUPPORTED
- numba不支持SM 12.0（RTX 5070）→ 添加到supported_ccs
- libdevice路径 → 设置NUMBAPRO_LIBDEVICE
...
dpd.setParams(type_i="A", type_j="B", alpha=30.0, sigma=4.5)  # A-B排斥更强
```
...
| A-A | 25.0 | 4.5 | 相同组分排斥 |
| B-B | 25.0 | 4.5 | 相同组分排斥 |
| A-B | 30.0-35.0 | 4.5 | 不同组分排斥（决定相分离强度） |
...
| 随机力系数 | `sigma` | `k_BT^(3/4) · m^(1/4) / r_c^(1/2)` | 4.5 | 涨落强度，需满足涨落-耗散 `sigma^2 = 2*gamma*k_BT` |
| 耗散系数 | `gamma` | `sqrt(m k_BT) / r_c` | 4.5（与 sigma 配对） | 摩擦系数 |
...
dpd = pygamd.force.dpd(info=snap, rcut=1.0)
dpd.setParams(type_i="A", type_j="A", alpha=25.0, sigma=4.5)  # 标准 Groot-Warren
dpd.setParams(type_i="B", type_j="B", alpha=25.0, sigma=4.5)  # 同组分
```
...
### nvvm.dll找不到
→ 使用`pygamd_gpu_init.py`，或确保CUDA bin目录在PATH中
...
### NumPy 2.x类型错误
→ 需要修复`snapshot.py`中的float转换（已包含在pygamd_gpu_init.py的补丁中）
...
| 13 | 删 `pygamd_gpu_init.py` 后跑 | 兼容性补丁丢失，CUDA 13.2+必崩 | 保留该文件在workspace根目录 |
...
- `pygamd_gpu_init.py` - GPU初始化模块（monkey-patch numba兼容性）
- `pygamd_dpd_gpu.py` - 完整DPD模拟示例（GPU模式）
- `ab_copolymer_dpd.py` - AB两嵌段共聚物自组装模拟
- `smart_visualize.py` - 智能OVITO可视化工具（自动检测/配色/采样）
- `visualize_trajectory.py` - 基础OVITO可视化脚本
- `test_gpu3.py` - GPU功能测试脚本
