...
/** Get a stable offset-based page without changing the legacy get-all API. */
export function getMessagesPage(
  sessionId: string,
  cursor?: string,
  limit = 0,
): { messages: ChatMessage[]; hasMore: boolean; nextCursor?: string } {
  const messages = getMessages(sessionId);
  const parsed = cursor === undefined ? 0 : Number.parseInt(cursor, 10);
  const offset = Number.isFinite(parsed) && parsed >= 0 ? parsed : 0;

  if (limit <= 0) {
    return { messages: messages.slice(offset), hasMore: false };
  }
