...
/** Extract text from the XML entries inside a .pptx ZIP archive. */
async function extractPptx(filePath: string): Promise<string> {
  const { default: JSZip } = await import('jszip');
  const zip = await JSZip.loadAsync(fs.readFileSync(filePath));
  const slideNames = Object.keys(zip.files)
    .filter((name) => /^ppt\/slides\/slide\d+\.xml$/i.test(name))
    .sort(naturalArchiveOrder);
  const parts: string[] = [];

  for (const name of slideNames) {
    const xml = await zip.file(name)!.async('string');
    parts.push(...extractXmlTagText(xml, 'a:t'));
  }
...
/** Extract shared strings, inline strings and values from an .xlsx archive. */
async function extractXlsx(filePath: string): Promise<string> {
  ...
  if (sharedEntry) {
    ...
    for (const item of items) sharedStrings.push(extractXmlTagText(item, 't').join(''));
  }
  ...
  for (const name of sheetNames) {
    ...
    for (const cell of cells) {
      ...
      if (type === 'inlineStr') {
        rows.push(extractXmlTagText(cell, 't').join(''));
        continue;
      }
      const rawValue = cell.match(/<v[^>]*>([\s\S]*?)<\/v>/i)?.[1];
      if (rawValue === undefined) continue;
      if (type === 's') {
        const index = Number.parseInt(rawValue, 10);
        if (Number.isFinite(index) && sharedStrings[index] !== undefined) rows.push(sharedStrings[index]);
      } else if (type === 'str') {
        rows.push(decodeXmlEntities(rawValue));
      } else {
        rows.push(rawValue);
      }
    }
  ...
  // Some producers create shared strings before worksheets. Keep them as a
  // fallback if no readable cells were present.
  const cleaned = cleanText((rows.length > 0 ? rows : sharedStrings).join('\n'));
  if (cleaned.length < 10) {
...
function naturalArchiveOrder(a: string, b: string): number {
