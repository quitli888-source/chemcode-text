...
//   - Tool invocations get their own <chemcode-tool-call-card> in the timeline
//   - Files become <chemcode-code-block> cards in the agent message
...
export class ChatView extends LitElement {
  static styles = css`
    ...
    .tool-summary-body chemcode-tool-call-card {
      margin: 3px 0;
    }
  ...
  private renderAgentMessage(m: ChatMessage, toolCalls: ChatMessage[]) {
    ...
    return html`
      <div class="message agent">
        ...
        <div class="msg-content">
          ${m.thinking
            ? html`<chemcode-thinking-block .content=${m.thinking} .live=${isStreaming}></chemcode-thinking-block>`
            : ''}
          <div class="msg-bubble">
            ${m.content
              ? html`<chemcode-markdown-renderer .source=${m.content}></chemcode-markdown-renderer>${isStreaming ? html`<span class="typing-cursor"></span>` : ''}`
              : isStreaming
  ...
  private async loadSkills() {
    ...
    try {
      const resp = await fetch('/api/skills', { headers: { 'Authorization': `Bearer ${localStorage.getItem('chemcode.token') || ''}` } });
      if (!resp.ok) return;
