...
//   - Memory-bounded: in-memory cache capped at MAX_CACHE_ENTRIES per user
...
/** Maximum entries cached in memory per user (prevents OOM with large knowledge bases). */
 ...
 *  Memory cache is capped at MAX_CACHE_ENTRIES to prevent OOM. */
export function getRecords(userId: string, offset = 0, limit = 0): KnowledgeRecord[] {
  ...
  // Cap memory: if records exceed MAX_CACHE_ENTRIES, keep the most recent ones.
  if (records.length > MAX_CACHE_ENTRIES) {
    const evicted = records.length - MAX_CACHE_ENTRIES;
    records.splice(0, evicted);
    console.warn(`[knowledge-store] cache capped: evicted ${evicted} oldest records for user ${userId}`);
  }
