...
function handleConnection(socket: WebSocket, req: IncomingMessage) {
  ...
  socket.on('close', () => {
    ...
    // errors are thrown.
    setTimeout(() => {
      // Check if this client has reconnected (a new Client with the same
      // userId would have been added to the set).
      ...
      if (!stillConnected && clients.has(client)) {
        ...
        console.log(`[ws] client reconnected, keeping runs alive for userId=${client.userId}`);
      }
  ...
  switch (cmd.type) {
    case 'user_message': {
      ...
      if (resolved) {
        ...
        // Use the frontend's agent message id when provided so that all
        // streamed events and the confirm key correlate with the bubble the
        // frontend already created. Fall back to a generated id otherwise.
        ...
        // Track the latest taskId created by the LLM during this run so we can
        // update its progress/completion status.
        let activeTaskId = '';
...
${e.content}`).join('\n\n---\n\n')}`;
          ...
          const { content: finalContent, usage, generatedFiles, thinking: finalThinking } = await runAgentLoop({
            ...
            if (ev.type === 'tool_call_end') {
              ...
              // Capture taskId from create_tool result so we can track progress.
              if (tc && tc.name === 'create_task' && tc.success && tev.result) {
                const m = tev.result.match(/ID:\s*(T-[\w-]+)/);
                if (m) activeTaskId = m[1];
              }
...
function persistAgentMessage(
  ...
  if (session) {
    ...
    // Tool call messages (rendered as <chemcode-tool-call-card>).
