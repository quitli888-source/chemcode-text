...
- Renamed the product, application directory, package, Web Components, browser storage keys, runtime memory directory, environment variables, defaults, and user-facing text to Chemcode.
- Standardized the data-directory configuration on the canonical v2 environment variable.
- Existing browser storage created by v1 is not migrated automatically because v2 uses the new canonical key namespace.

### Documentation

- Rebuilt the root and application README files around the current v2 architecture and PyGAMD workflow.
- Replaced stale development and usage instructions with current commands, limits, confirmation behavior, and project paths.
