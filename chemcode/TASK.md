...
Chemcode（原 ChemAgent）是一个面向计算化学/分子动力学/量子化学的 LLM 驱动 Agent 平台。
...
- `server/src/tools/save-note.ts` — 写入 `.chemcode-memory/` 笔记
...
- **记忆文件夹**：自动创建 `.chemcode-memory/`（含 `notes.md`、`history.log`）
